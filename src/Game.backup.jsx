...
function PlayingPanel({ question, myData, partnerName, myColor, myGradient, partnerColor, onSubmitAnswer }) {
  ...
  return (
    <>
      ...
      {/* Answer buttons */}
      {!currentAnswer ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} className="scale-in">
          <AnswerButton
            label="A"
        ...
        <div className="scale-in" style={{
          ...
          <div style={{
            ...
            padding: '6px 16px',
            borderRadius: 20,
            background: currentAnswer === 'A' ? myGradient : 'linear-gradient(135deg, #4facfe, #00f2fe)',
            fontSize: 14,
            fontWeight: 800,
            color: '#fff',
            marginBottom: 16,
          }}>
            {currentAnswer} — {currentAnswer === 'A' ? question.optionA : question.optionB}
          </div>
...
function RevealPanel({
  ...
  return (
    <div key={currentRound} className="fade-slide" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
...
function AnswerCard({ name, answer, question, gradient, matched }) {
  const text = answer === 'A' ? question.optionA : answer === 'B' ? question.optionB : '?'
